...
//GLOBALE VARIABLEN__________________________________________________________________________________________________________________________________	
		...
		//Alles über Zeugen
				...
				//child	
					...
					public static String childaussagen[]=
							...
							/*relation[1]*/ "The lord gave me sweats sometimes! I Looooove sweats",
							/*weird[2]*/    "Nope",
							/*susp[3]*/     "The Dinosaur!",
	...
	//RegEX-----------------------------------------------------------------------------------------------------------------
			...
			//watson
				...
				public static String askwwi="where was i|where have i been|room(s)? (did i)? (visit(ed)?|go to)|have i (been to|visited)|room(s)? (are|is) left";
	...
	//Ausgabe---------------------------------------------------------------------------------------------------------------------------------------
		...
		//sound() bekommt entweder den link zu einer entsprechenden MP3 oder einen schon im Programm vorhandenen Sound
			public static void sound(String link) {
				if (link=="rain"){
						link="soundbank://soundlibrary/weather/rain/rain_03";
				}
				else if(link=="thunder"){
					link="soundbank://soundlibrary/weather/thunder/thunder_04";
				}
				else if(link=="cat"){
					Random r = new Random();
					int z=r.nextInt(5);
			...
			public static void solve(int zeit) {
				...
				case 1://Wer war es?/Leave
					...
					if(yn) {
						...
						else if(inbefragung==true){logger.info("solve("+zeit+"):"+"Es wurde richtung watson verlassen"); zeuge(aktuellerzeuge,0);}
						else {raum(aktuellerraum,0);}
					}
					break;
					...
					case 1://frageauswahl
						...
						switch(Frage) {
							...
							case 2://where was I?
								...
								String[] ww=wwi();
								String wwi=ww[0];//where was I
								String wl=ww[1];//whats left
								speak("You have already been to "+wwi+". That means, that you still have to go to this places"+wl, watson);
								speak("How else can I help you?", "watson");
								watsonzeit=1;
								break;
							case 3://solve
